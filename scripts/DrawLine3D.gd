...
func _draw():
	...
	var i = Lines.size() - 1
	while (i >= 0):
		if(Lines[i].time != null and Lines[i].time < 0.0):
			Lines.remove_at(i)
			RemovedLine = true
		i -= 1

func DrawLine(Start, End, LineColor, time = null):
	Lines.append(Line.new(Start, End, LineColor, time))

func DrawRay(Start, Ray, LineColor, time = null):
	Lines.append(Line.new(Start, Start + Ray, LineColor, time))

func DrawCube(Center, HalfExtents, LineColor, time = null):
